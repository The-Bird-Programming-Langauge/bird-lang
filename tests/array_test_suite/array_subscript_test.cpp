...
TEST(ArrayTestSuite, FloatArraySubscript) {
  ...
  options.after_interpret = [&](Interpreter &interpreter) {
    ...
    ASSERT_EQ(
        ...
        1.1);
    ASSERT_EQ(
        as_type<double>(as_type<ARRAY_TYPE>(interpreter.env.get("x"))->at(1)),
        2.2);
    ASSERT_EQ(
        as_type<double>(as_type<ARRAY_TYPE>(interpreter.env.get("x"))->at(2)),
        3.3);
  };
...
TEST(ArrayTestSuite, StrArraySubscript) {
  ...
  options.after_interpret = [&](Interpreter &interpreter) {
    ...
    ASSERT_EQ(as_type<std::string>(
                  as_type<ARRAY_TYPE>(interpreter.env.get("x"))->at(0)),
              "hello");
    ASSERT_EQ(as_type<std::string>(
                  as_type<ARRAY_TYPE>(interpreter.env.get("x"))->at(1)),
              ", ");
    ASSERT_EQ(as_type<std::string>(
                  as_type<ARRAY_TYPE>(interpreter.env.get("x"))->at(2)),
              "world!");
  };
