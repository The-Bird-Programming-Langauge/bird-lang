...
TEST(MiscProgramsTestSuite, MatrixMultiplicationTest)
{
  ...
  options.after_interpret = [&](Interpreter &interpreter)
  {
    ...
    const auto result_is_array =
        is_type<ArrayType>(interpreter.current_namespace->environment.get("result"));
    ASSERT_TRUE(result_is_array);

    const auto result = as_type<ArrayType>(interpreter.current_namespace->environment.get("result"));
