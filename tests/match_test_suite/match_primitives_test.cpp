...
TEST(MatchTest, MatchString) {
  ...
  options.after_interpret = [&](Interpreter &interpreter) {
    ...
    ASSERT_EQ(as_type<int>(interpreter.env.get("y")), 1);
  };
...
TEST(MatchTest, MatchInt) {
  ...
  options.after_interpret = [&](Interpreter &interpreter) {
    ASSERT_TRUE(interpreter.env.contains("y"));
    ASSERT_TRUE(is_type<std::string>(interpreter.env.get("y")));
    ASSERT_EQ(as_type<std::string>(interpreter.env.get("y")), "one");
  };
...
TEST(MatchTest, MatchBool) {
  ...
  options.after_interpret = [&](Interpreter &interpreter) {
    ASSERT_TRUE(interpreter.env.contains("y"));
    ASSERT_TRUE(is_type<std::string>(interpreter.env.get("y")));
    ASSERT_EQ(as_type<std::string>(interpreter.env.get("y")), "true");
  };
