...
TEST(ExprTest, BinaryExpr)
{
  ...
  options.after_interpret = [](Interpreter &interpreter)
  {
    ...
    ASSERT_EQ(as_type<int>(interpreter.current_namespace->environment.get("x")), -8);
  };
...
TEST(ExprTest, BinaryStringString)
{
  ...
  options.after_interpret = [](Interpreter &interpreter)
  {
    ...
    ASSERT_EQ(as_type<std::string>(interpreter.current_namespace->environment.get("x")), "hellothere");
  };
...
TEST(ExprTest, IdentifierInExpr)
{
  ...
  options.after_interpret = [](Interpreter &interpreter)
  {
    ASSERT_TRUE(interpreter.current_namespace->environment.contains("x"));
    ASSERT_TRUE(is_type<int>(interpreter.current_namespace->environment.get("x")));
    ASSERT_EQ(as_type<int>(interpreter.current_namespace->environment.get("x")), 55);
  };
