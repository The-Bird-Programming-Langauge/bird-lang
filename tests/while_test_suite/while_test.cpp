...
TEST(WhileTest, While)
{
  ...
  options.after_interpret = [&](Interpreter &interpreter)
  {
    ...
    ASSERT_EQ(as_type<int>(interpreter.current_namespace->environment.get("x")), 10);
  };
...
{
  BirdTest::TestOptions options;
  options.code = "var x = 0; while true { x += 1; if (x <= 2) { continue; } "
                 "print x; break; print 4; }";

  options.after_interpret = [&](Interpreter &interpreter)
  {
    ASSERT_TRUE(interpreter.current_namespace->environment.contains("x"));
    ASSERT_TRUE(is_type<int>(interpreter.current_namespace->environment.get("x")));
    ASSERT_EQ(as_type<int>(interpreter.current_namespace->environment.get("x")), 3);
  };
...
TEST(WhileTest, WhileConstInc)
{
  BirdTest::TestOptions options;
  options.code =
      "const inc = 1; var x = 0; while x < 10 { x += inc; print x; }";
  ...
  options.after_interpret = [&](Interpreter &interpreter)
  {
    ...
    ASSERT_TRUE(interpreter.current_namespace->environment.contains("inc"));
    ASSERT_TRUE(is_type<int>(interpreter.current_namespace->environment.get("inc")));
    ASSERT_EQ(as_type<int>(interpreter.current_namespace->environment.get("inc")), 1);
  };
