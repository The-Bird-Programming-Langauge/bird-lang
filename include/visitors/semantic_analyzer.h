...
public:
  std::shared_ptr<Namespace<SemanticValue, SemanticCallable, SemanticType>>
      global_namespace;
  std::shared_ptr<Namespace<SemanticValue, SemanticCallable, SemanticType>>
      current_namespace;
  ...
  SemanticAnalyzer(UserErrorTracker &user_error_tracker)
      : global_namespace(
            std::make_shared<
                Namespace<SemanticValue, SemanticCallable, SemanticType>>(
                nullptr)),
  ...
  void visit_assign_expr(AssignExpr *assign_expr) {
    if (!this->current_namespace->environment.contains(
        ...
        !this->current_namespace->call_table.contains(
            assign_expr->identifier.lexeme)) {
  ...
  void visit_type_stmt(TypeStmt *type_stmt) {
    ...
    this->current_namespace->type_table.declare(type_stmt->identifier.lexeme,
                                                SemanticType());
  }
