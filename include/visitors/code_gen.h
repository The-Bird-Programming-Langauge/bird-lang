...
static unsigned int
bird_type_byte_size(std::shared_ptr<BirdType> type) // in i32s
{
  switch (type->type) {
...
public:
  ...
  ~CodeGen() { BinaryenModuleDispose(this->mod); }
  ...
  void init_std_lib() {
    ...
    BinaryenAddFunctionImport(this->mod, "print_endline", "env",
                              "print_endline", BinaryenTypeNone(),
                              BinaryenTypeNone());
  ...
  void init_static_memory(std::vector<std::string> &strings) {
    ...
    BinaryenSetMemory(mod,
                      ...
                      "memory", segments.data(), passive, offsets.data(),
                      sizes.data(), segments.size(), 0);
  ...
  void generate(std::vector<std::unique_ptr<Stmt>> *stmts) {
    ...
    for (auto &local : this->function_locals["main"]) {
      if (local == BinaryenTypeInt32()) {
        BinaryenAddGlobal(this->mod, std::to_string(count++).c_str(), local,
                          true,
                          BinaryenConst(this->mod, BinaryenLiteralInt32(0)));
      } else if (local == BinaryenTypeFloat64()) {
        BinaryenAddGlobal(
            this->mod, std::to_string(count++).c_str(), local, true,
  ...
  bool is_bird_type(Token token) {
    return token.lexeme == "bool" || token.lexeme == "int" ||
           token.lexeme == "float" || token.lexeme == "void" ||
           token.lexeme == "str";
  }
  ...
  BinaryenType from_bird_type(std::shared_ptr<BirdType> token) {
    if (token->type == BirdTypeType::BOOL)
      ...
      return BinaryenTypeNone();
    else if (token->type == BirdTypeType::STRING ||
             token->type == BirdTypeType::STRUCT ||
             token->type == BirdTypeType::ARRAY)
      return BinaryenTypeInt32();
    else {
  ...
  // perform garbage collection on memory data by marking and sweeping
  // dynamically allocated blocks
  void garbage_collect() {
    // list that stores all of the javascript calls to be pushed on the stack as
    // 1 block
    std::vector<BinaryenExpressionRef> calls;

    // mark all dynamically allocated blocks by traversing the environment,
    // locate all pointers pointing to dynamically allocated blocks, and pass
    // the pointers to the mark function
    ...
    for (const auto &scope : this->environment.envs) {
      for (const auto &[key, value] : scope) {
        if (value.type->type == BirdTypeType::STRUCT &&
          ...
          calls.push_back(BinaryenCall(this->mod, "mark", &allocated_block_ptr,
                                       1, BinaryenTypeNone()));
        }
      }
    }
  ...
  void visit_decl_stmt(DeclStmt *decl_stmt) {
    ...
    BinaryenExpressionRef set_local = this->binaryen_set(
        decl_stmt->identifier.lexeme, initializer_value.value);
  ...
  void visit_while_stmt(WhileStmt *while_stmt) {
    ...
    auto outer_body = BinaryenBlock(this->mod, "BODY", children.data(),
    ...
    while_body_children.push_back(
        BinaryenBreak(this->mod, "LOOP", condition.value, nullptr));

    auto while_body =
        BinaryenBlock(this->mod, "while_block", while_body_children.data(),
                      while_body_children.size(), BinaryenTypeNone());

    auto if_cond =
        BinaryenIf(this->mod, condition.value,
                   BinaryenLoop(this->mod, "LOOP", while_body), nullptr);

    this->stack.push(
        BinaryenBlock(this->mod, "EXIT", &if_cond, 1, BinaryenTypeNone()));
  }
  ...
  void visit_for_stmt(ForStmt *for_stmt) {
    ...
    auto body_and_condition =
        BinaryenBlock(this->mod, "BODY", children.data(), children.size(),
                      BinaryenTypeNone());
  ...
  void visit_binary_normal(Binary *binary) {
    ...
    try {
      ...
      std::cout << "right type: " << bird_type_to_string(right.type)
                << std::endl;
  ...
  void visit_unary(Unary *unary) {
    ...
    case Token::Type::QUESTION: {
      this->stack.push(TaggedExpression(
          BinaryenSelect(this->mod, expr.value,
    ...
    case Token::Type::MINUS: {
      if (expr_type == BinaryenTypeFloat64()) {
        this->stack.push(TaggedExpression(
            BinaryenUnary(mod, BinaryenNegFloat64(), expr.value),
            std::shared_ptr<BirdType>(new BoolType())));
  ...
  void visit_primary(Primary *primary) {
    ...
    case Token::Type::STR_LITERAL: {
      ...
      this->stack.push(TaggedExpression(
          str_ptr, std::shared_ptr<BirdType>(new StringType())));
      break;
    }

    case Token::Type::IDENTIFIER: {
      TaggedIndex tagged_index = this->environment.get(primary->value.lexeme);
      BinaryenExpressionRef local_get =
          this->binaryen_get(primary->value.lexeme);
  ...
  void visit_const_stmt(ConstStmt *const_stmt) {
    ...
    if (const_stmt->type.has_value()) {
      ...
      if (initializer.type->type != BirdTypeType::VOID) {
        ...
        BinaryenType binaryen_type =
            BinaryenExpressionGetType(initializer.value);
        type = (binaryen_type == BinaryenTypeFloat64())
                   ? std::shared_ptr<BirdType>(new FloatType())
                   : std::shared_ptr<BirdType>(new IntType());
      }
    ...
    environment.declare(const_stmt->identifier.lexeme,
                        TaggedIndex(index, type));
  ...
  void visit_func(Func *func) {
    ...
    if (func->return_type.has_value()) {
      ...
      auto binaryen_return_type =
          this->bird_type_to_binaryen_type(bird_return_type);
    ...
    for (auto &param : func->param_list) {
      ...
      this->function_locals[func_name].push_back(
          this->bird_type_to_binaryen_type(param_type));
    }
    ...
    BinaryenType result_type =
        func->return_type.has_value()
            ? this->bird_type_to_binaryen_type(
                  this->type_converter.convert(func->return_type.value()))
            : BinaryenTypeNone();
  ...
  void visit_break_stmt(BreakStmt *break_stmt) {
    this->stack.push(BinaryenBreak(this->mod, "EXIT", nullptr, nullptr));
  }
  ...
  void visit_subscript(Subscript *subscript) {
    ...
    if (subscriptable.type->type == BirdTypeType::ARRAY) {
      type = safe_dynamic_pointer_cast<ArrayType>(subscriptable.type)
                 ->element_type;
    }
    ...
    BinaryenExpressionRef args[2] = {
        ...
        subscriptable.type->type == BirdTypeType::ARRAY
            ? BinaryenBinary(
                  ...
                  BinaryenConst(this->mod, BinaryenLiteralInt32(
                                               bird_type_byte_size(type))))
            : index.value};
  ...
  */
    ...
    if (struct_constructors.find(struct_initialization->identifier.lexeme) ==
      ...
      for (auto &field : struct_type->fields) {
        ...
        BinaryenExpressionRef args[3] = {
            BinaryenLocalGet(this->mod, param_types.size(),
                             BinaryenTypeInt32()),
            BinaryenConst(this->mod, BinaryenLiteralInt32(offset)),
      ...
      constructor_body.push_back(BinaryenReturn(
          this->mod, BinaryenLocalGet(this->mod, param_types.size(),
                                      BinaryenTypeInt32())));
  ...
  void visit_member_assign(MemberAssign *member_assign) {
    ...
    BinaryenExpressionRef args[3] = {
        accessable.value,
        BinaryenConst(this->mod, BinaryenLiteralInt32(offset)), value.value};
  ...
  BinaryenExpressionRef binaryen_set(std::string identifier,
    ...
    // std::cout << identifier << ": " <<
    // this->environment.get_depth(identifier) << std::endl;
    ...
    if (this->environment.get_depth(identifier) != 0 &&
      ...
      return BinaryenGlobalSet(
          this->mod, std::to_string(tagged_index.value).c_str(), value);
    }
  }
  ...
  void visit_array_init(ArrayInit *array_init) {
    ...
    this->stack.push(
        TaggedExpression(block, std::make_shared<ArrayType>(type)));
  }
