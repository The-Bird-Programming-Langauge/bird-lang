...
public:
  ...
  std::shared_ptr<
      Namespace<std::shared_ptr<BirdType>, std::shared_ptr<BirdFunction>,
  ...
  TypeChecker(UserErrorTracker &user_error_tracker)
      : user_error_tracker(user_error_tracker),
        global_namespace(
            std::make_shared<Namespace<std::shared_ptr<BirdType>,
  ...
  void visit_assign_expr(AssignExpr *assign_expr) {
    ...
    auto previous = this->current_namespace->environment.get(
        assign_expr->identifier.lexeme);
  ...
  void visit_const_stmt(ConstStmt *const_stmt) {
    ...
    if (result->type == BirdTypeType::VOID) {
      ...
      this->current_namespace->environment.declare(
          const_stmt->identifier.lexeme, std::make_shared<ErrorType>());
      return;
    }
  ...
  void visit_primary(Primary *primary) {
    ...
    case Token::Type::IDENTIFIER: {
      this->stack.push(
          this->current_namespace->environment.get(primary->value.lexeme));
      break;
    }
    default: {
  ...
  void visit_call(Call *call) {
    // auto function =
    //     this->current_namespace->call_table.get(call->identifier.lexeme);
  ...
  void visit_type_stmt(TypeStmt *type_stmt) {
    if (this->current_namespace->type_table.contains(
  ...
  visit_struct_initialization(StructInitialization *struct_initialization) {
    ...
    while (this->current_namespace &&
           !this->current_namespace->type_table.contains(
    ...
    auto type = this->current_namespace->type_table.get(
        struct_initialization->identifier.lexeme);
      ...
      for (auto &field_assignment : struct_initialization->field_assignments) {
        ...
        if (field.first == field_assignment.first) {
          ...
          if (field.second->type == BirdTypeType::PLACEHOLDER) {
            ...
            field.second =
                this->current_namespace->type_table.get(placeholder->name);
          }
