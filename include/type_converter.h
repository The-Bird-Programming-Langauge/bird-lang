...
public:
    TypeConverter(Environment<std::shared_ptr<BirdType>> &type_table, std::set<std::string> &struct_names)
    ...
    std::shared_ptr<BirdType> convert(std::shared_ptr<ParseType::Type> type)
    {
        ...
        {
            Token token = safe_dynamic_pointer_cast<ParseType::Primitive, ParseType::Type>(type)->type;
        ...
        {
            Token token = safe_dynamic_pointer_cast<ParseType::UserDefined, ParseType::Type>(type)->type;
            auto type_name = token.lexeme;
