...
BinaryenExpressionRef CodeGen::binaryen_set(std::string identifier, BinaryenExpressionRef value)
{
...
BinaryenExpressionRef CodeGen::binaryen_get(std::string identifier)
{
    ...
    if (this->environment.get_depth(identifier) != 0 && this->current_function_name != "main")
    {
        return BinaryenLocalGet(this->mod, tagged_index.value, bird_type_to_binaryen_type(tagged_index.type));
    }
    else
    {
        return BinaryenGlobalGet(this->mod, std::to_string(tagged_index.value).c_str(), bird_type_to_binaryen_type(tagged_index.type));
    }
}
